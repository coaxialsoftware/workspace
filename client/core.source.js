((ide, cxl, codeMirror) => {
	...
	class SourceHintsFeature extends ide.feature.HintsFeature {
		...
		__getMarker(line) {
			...
			return (
				h &&
				((h.gutterMarkers && h.gutterMarkers['editor-hint-gutter']) ||
		...
		add(hint) {
			...
			el.setAttribute(
				'class',
				'editor-hint bg-' + (hint.className || 'info')
			);
	...
	class SourceInsertFeature extends ide.feature.InsertFeature {
		...
		tab() {
			// TODO
			this.editor.editor.execCommand(
				this.editor.options.indentWithTabs
					? 'insertTab'
					: 'insertSoftTab'
			);
		}
		...
		disable() {
			...
			if (cm.getOption('disableInput') === false) {
				...
				if (cm.getCursor().ch > 0) cm.execCommand('goCharLeft');
	...
	class SourceCursorFeature extends ide.feature.CursorFeature {
		...
		enter() {
			if (this.editor.insert.enabled) this.editor.insert.line();
			else this.goDown();
		}
	}
	...
	class SourceScrollFeature extends ide.feature.ScrollFeature {
		...
		scrollLineUp(n, dir) {
			var ed = this.editor,
				...
				y =
					(dir || -1) *
					Math.round(n ? h * n : scroll.clientHeight / 2);
	...
	class SourceSelectionFeature extends ide.feature.SelectionFeature {
		...
		clear() {
			this.editor.editor.setSelection(
				this.editor.editor.getCursor('anchor')
			);
		}
		...
		get value() {
			return this.editor.editor.getSelection(
				this.editor.options.lineSeparator
			);
		}
	...
	class SourceHistoryFeature extends ide.feature.HistoryFeature {
		get lastInsert() {
			...
			while (l--)
				if (history[l].changes)
					return cm.getRange(
						history[l].changes[0].from,
						history[l].changes[0].to
					);
		}
		...
		getAll() {
			...
			return (
					...
					return new ide.HistoryRecord(
						h.changes ? 'edit' : 'selection'
					);
				})
			);
		}
	...
	class SourceLineFeature extends ide.feature.LineFeature {
		...
		get value() {
			return this.editor.editor.getLine(
				this.editor.editor.getCursor().line
			);
		}
	...
	class SourceWordFeature extends ide.feature.WordFeature {
		get current() {
			...
			if (row === this.$row && col === this.$col) return this.$current;
	...
	class SourceRange extends ide.Token {
		...
		getCoordinates() {
			return this.editor.editor.charCoords({
				line: this.row,
				ch: this.column,
			});
		}
	...
	class SourceTokenFeature extends ide.feature.TokenFeature {
		...
		render() {
			this.editor.listenToCM(
				'cursorActivity',
				this._onCursorActivity.bind(this)
			);
			this.current = this.getToken(0, 0);
		}
		...
		getToken(row, column, result) {
			...
			result.cursorValue = token.string.substr(
				0,
				result.cursorColumn - token.start
			);
			result.$token = token;
		...
		_onCursorActivity() {
			const pos = this.editor.editor.getCursor();
	...
	class DocumentFeature extends ide.Feature {
		getRange() {
			return this.editor.range.create(
				0,
				0,
				this.editor.editor.doc.lastLine()
			);
		}
	}
	...
	class SourceEditor extends ide.FileEditor {
		$setFocus() {
			...
			if (ide.editor !== this) ide.workspace.focusEditor(this);
		}

		cmd(fn, args) {
			if (!isNaN(fn)) return this.cursor.go(fn);
		...
		_findMode() {
			var filename = this.file.name,
				mime = this.file.mime,
				info =
					(filename && codeMirror.findModeByFileName(filename)) ||
			...
			if (!codeMirror.modes[mode]) {
				...
				if (info.require) promises.push(getScript(info.require));
		 ...
		 */
		_keymapHandle(key) {
			var maps = this.editor.state.keyMaps,
				l = maps.length,
				fn,
				result;
			while (l--) {
				if ((fn = maps[l][key])) {
					...
					if (result !== codeMirror.Pass) return result;
				}
			}
		...
		render(p) {
			super.render(p);
			var ft = (this.mode = this._findMode()),
