((ide, cxl) => {
	...
	cxl.extend(CommandParser.prototype, {
		...
		ignoreSpace(args, state) {
			while (/\s/.test(args[state.i])) state.i++;
		},
		...
		parseArguments(args, state) {
			...
			while (state.i < state.end && args[state.i] !== ';') {
				switch (args[state.i]) {
					case '"':
						this.parseString(args, state);
						break;
					case '/':
						this.parseRegex(args, state);
						break;
					case '`':
						this.parseJS(args, state);
						break;
					default:
						this.parsePath(args, state);
						break;
				}
		...
		parse(src, silent) {
			var state = { i: 0, result: [], end: src.length, silent: silent },
				commands = [],
				current;
			do {
				current = {
		...
		run(src) {
			const cmd = this.parse(src.toString());
	...
	ide.run = function (fn, args) {
		...
		try {
			...
			if (result === ide.Pass) result = tryCmd(ide.commands, fn, args);

			if (result instanceof ide.Editor) {
				// TODO ?
				if (!result.command) result.command = fn;
				if (!result.arguments) result.arguments = args;
	...
	ide.plugins.register('core', {
		...
		editorCommands: {
			...
			'workspace.swapNext': function () {
				var l = ide.workspace.slots.length,
					i;
				if (l > 1) {
		...
		commands: {
			browse(url) {
				...
				if (!BROWSER_REGEX.test(url)) url = 'http://' + url;
			...
			commands: {
				fn: function () {
					...
					editor.listenTo(
						ide.plugins,
						'editor.focus',
						this.loadCommands.bind(this, editor)
					);
			...
			keymap() {
				...
				editor.listenTo(
					ide.plugins,
					'editor.focus',
					this.loadKeymap.bind(this, editor)
				);
				editor.listenTo(
					ide.plugins,
					'editor.keymap',
			...
			help(topic) {
				var url =
					ide.project.get('help.url') + (topic ? '#' + topic : '');
			...
			'plugins.install': {
				fn: function (id) {
					...
					return cxl.ajax
						...
						.catch(() =>
							ide.error(`Error installing ${id} extension`)
						)
						.then(() => not.remove());
				},
			...
			'q!'() {
				if (ide.editor) ide.editor.quit(true);
				else window.close();
			},
		...
		getAllCommands: function (search) {
			var result = [],
				len = search && search.length;

			function getCommands(cmds) {
				var key, fn, index;

				for (var i in cmds) {
					if (search && (index = i.indexOf(search)) === -1) continue;
					...
					result.push(
						new ide.Item({
							...
							priority: index,
							matchStart: search && index,
							matchEnd: search && index + len,
						})
					);
				}
			}
		...
		loadKeymap: function (editor) {
			var state = ide.editor.keymap.state,
				items = [];
			editor.reset();

			editor.header.title = 'keymap: ' + (state || '');
			items = items.concat(
				this.getKeys(ide.editor.keymap.getState(state))
			);
	...
	ide.Command = class Command {
		constructor(name, def, scope, prop) {
			...
			if (type === 'string' || type === 'function') def = { fn: def };

			if (typeof def.fn === 'string') {
				this.fn = function () {
					ide.run(def.fn, arguments);
				};
		...
		parse(def) {
			...
			if (!def.cmd) this.fn.help = def.help;
		}

		getToken(editor, token) {
			var result = [],
				ch = token.string,
				args = token.state.args,
				l = args.length - 1,
		...
		match(args, exact, def) {
			var i = 0,
				...
				arg,
				editor = ide.editor;
			if ((def.editor && !editor) || (l > 0 && !cmd)) return false;
			...
			if (l > 0 && cmd)
				for (; i < l; i++) {
					...
					// Parameter
					if (cur && cur[0] === '@') match[cur.substr(1)] = arg;
					else if (
						!cur ||
						(exact && cur !== arg) ||
						cur.indexOf(arg) !== 0
