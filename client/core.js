...
(cxl => {
	...
	function findPlugin(options) {
		if (options.plugin) {
			return options.command
				? options.plugin.commands[options.command].call(
						options.plugin,
						options.params
				  )
	...
	function loadEditor(options, file) {
		...
		if (options.focus !== false) ide.workspace.focusEditor(editor);

		return editor;
	}

	function onOpenError(options, e) {
		if (options.slot) ide.workspace.removeSlot(options.slot);
	...
	ide.open = function (options) {
		...
		else if (options instanceof ide.File) options = { file: options };
		...
		return (options.file
			? loadFile(options.file).then(loadEditor.bind(ide, options))
			: Promise.resolve(loadEditor(options))
		).catch(onOpenError.bind(ide, options));
	};
	...
	if (document.readyState !== 'loading') window.setTimeout(_start);
	else window.addEventListener('DOMContentLoaded', _start);
