(function (window, ide, cxl) {
	...
	cxl.extend(InlineAssist.prototype, {
		...
		clearHints(editor) {
			if (this.hints.length) this.hints = [];
		...
		calculateLeft(editor) {
			var pos = (this.pos = editor.token.current.getCoordinates()),
				// TODO ...
		...
		calculateTop() {
			var el = this.el,
				...
				translate =
					'translate(' + this.leftPos + 'px,' + height + 'px)';
		...
		addHints(hints) {
			...
			for (; i < l; i++) this.add(hints[i]);

			if (!this.visible) this.show(this.editor);
		...
		renderHint(hint, i) {
			...
			if (this.selectedValue === hint.value) this.select(hint);
		...
		_renderDown(el, hints, i, l) {
			if (this.visibleStart > 0) el.appendChild(this.scrollUpEl);

			for (; i < l; i++) this.renderHint(hints[i], i);
		...
		_renderUp(el, hints, i, l) {
			...
			for (l--; l >= i; l--) this.renderHint(hints[l], l);

			if (this.visibleStart > 0) el.appendChild(this.scrollDownEl);
		},

		_render() {
			var i = this.visibleStart,
				l = (this.visibleEnd = i + this.visibleCount),
				el = this.hintsContainer,
				hints;
			if (l > this.hints.length) l = this.hints.length;
			if (l === 0) return this.hide();
			...
			if (this.isDown) this._renderDown(el, hints, i, l);
			else this._renderUp(el, hints, i, l);
		...
		_goNext(dir) {
			var selected = this.selected,
				...
				diff =
					(this.isDown ? 1 : -1) *
					(dir === 'previousSibling' ? -1 : 1),
				nextIndex = index + diff,
				next = this.hints[nextIndex];
			if (next) {
				if (
					nextIndex >= this.visibleEnd ||
					nextIndex < this.visibleStart
				) {
		...
		doAccept() {
			...
			if (value && token) token.replace(value);
		...
		accept() {
			...
			if (this.hints.length === 0) return ide.Pass;

			if (
				this.selectedValue &&
				token.cursorValue === this.selectedValue
			) {
				this.hide();
	...
	class AssistPanel {
		constructor() {
			...
			this.el.innerHTML =
				'<div class="assist-perm"></div><div class="assist-hints"></div>';
		...
		onItemClick(ev) {
			let el = ev.target;
			while (el && el.tagName !== 'IDE-ITEM') {
				el = el.parentNode;
			}

			const action = el && el.$item && el.$item.action;

			if (action) ide.commandParser.run(action);
		}
		...
		sortedLastIndexBy(hint) {
			...
			while (l--) {
				if (hint.priority > this.hints[l].priority) return l;
			}
		...
		addHint(hint) {
			if (this.rendered) this.renderHint(hint);
			else this.hints.push(hint);
		}
		...
		renderHint(hint, i) {
			...
			} else this.$hints.appendChild(el);
		}
	...
	class Assist {
		constructor() {
			...
			this._debouncedRequest = cxl.debounce(
				this._requestHints,
				this.delay
			);
		...
		onResponseExtended(version, hints) {
			...
			if (/*version!==me.version ||*/ !me.panel.visible || !hints) return;
	...
	ide.plugins.register(
		...
		new ide.Plugin({
			...
			ready() {
				...
				this.listenTo(
					'socket.message.assist.extended',
					this.onExtended
				);

				if (ide.hash.data.a) ide.assist.panel.show();
			},
		})
	);
