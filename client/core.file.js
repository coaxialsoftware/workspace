((cxl, ide) => {
	...
	const CONTENT_TYPE_REGEX = /([\w\d\/\-]+)(?:;\s*charset="?([\w\d\-]+)"?)?/,
		ICON_PROVIDERS = [];
	...
	class File {
		static joinPath() {
			return Array.prototype.join.call(
				arguments,
				ide.project.get('path.separator') || '/'
			);
		}
		...
		static getIcon(file) {
			return (
				findIconByProvider(file) ||
				(file.mime === 'text/directory' ? 'directory' : 'file')
			);
		}
		...
		$url() {
			var // TODO remove this and use etag
				mtime =
					this.stat && this.stat.mtime
						? '&t=' + this.stat.mtime.getTime()
						: '';
			return (
				'file?p=' +
				encodeURIComponent(ide.project.id) +
				'&n=' +
		...
		$parse(xhr) {
			const m = CONTENT_TYPE_REGEX.exec(
			...
			this.stat = this.$parseStat(
				JSON.parse(xhr.getResponseHeader('ws-file-stat'))
			);
		...
		$onError(xhr) {
			var res = xhr.response || xhr.statusText || xhr.message,
				id = this.id || ide.project.id + '/' + this.name,
				msg =
					this.decode(res, 'utf8') ||
					(this.saving
						? 'Error saving file: '
						: 'Error opening file: ') + id;
			this.$fetching = null;
		...
		read(encoding) {
			...
			return (this.$fetching = cxl.ajax
				.xhr({ url: url, responseType: 'arraybuffer' })
		...
		delete() {
			return cxl.ajax
				.xhr({ url: this.$url(), method: 'DELETE' })
				.then(() => this.read())
				.catch(this.$onError.bind(this));
		}
	...
	class FileItem extends ide.Item {
		...
		enter(shift, mod) {
			...
			if (item.line) options.line = item.line;

			if (mod) options.target = '_blank';
	...
	class FileEditorHeader extends ide.feature.EditorHeader {
		...
		getTitle() {
			var plugin = this.editor.plugin && this.editor.plugin.name,
				filename = this.editor.file.name || 'No Name';
			return (plugin ? plugin + ':' : '') + filename;
		}

		update() {
			var changed = this.editor.file.hasChanged(),
				title = this.getTitle();
			...
			if (this.editor.file.$sync.outOfSync)
				this.setTag(
					'file.old',
					'<span title="File contents have changed">Out of Sync</span>',
					'error'
				);

			this.setTag(
				'file.new',
				this.editor.file.stat.isNew ? 'New' : null,
				'success'
			);
		...
		get() {
			var editor = this.editor,
				...
				args = editor.arguments
					? this.serializeArgs(editor.arguments)
					: editor.file.name || '';
			return (cmd ? cmd + ':' : '') + args;
		}
	}
	...
	class FileDiff {
		...
		diff() {
			const old = this.originalContent,
				cur = this.$file.content,
				changed = (this.diffChanged =
					!this.lastDiff || this.diffValue !== cur);
			if (changed) {
				this.diffValue = cur;
				return (this.lastDiff = ide.diffPromise(old, cur));
			} else return Promise.resolve(this.lastDiff);
		}
	}
	...
	class FileFeature extends ide.Feature {
		...
		delete() {
			if (!this.name) return ide.warn('File has no name');
	...
	class FileFormatFeature extends ide.Feature {
		set(format) {
			...
			if (format === 'unix')
				...
				content = file.content.replace(/\r?\n/g, '\r');
			else throw new Error('format is required');
	...
	function getTags(file) {
		...
		if (file.stat.isDirectory) tags.push('directory');
		else if (file.mime) tags.push(file.mime);

		return tags;
	}

	ide.plugins.on('assist', function (request) {
		if (!request.extended) return;

		var file = request.editor && request.editor.file;

		if (!file) return;

		if (!fileItem.el) ide.assist.addPermanentItem(fileItem);
		...
		if (file.$sync.outOfSync)
			request.respondExtended({
				title: 'File contents have changed',
				code: 'file',
				className: 'error',
			});
	});
	...
	Object.assign(ide.feature, {
		...
		FileFormatFeature: FileFormatFeature,
	});
})(this.cxl, this.ide);
