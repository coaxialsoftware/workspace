...
class Plugin {
	...
	load() {
		...
		// TODO ...umm
		this.id = mod.name.replace(/^workspace\./, '').replace(/\./g, '-');
		this.name = mod.name;
		...
		if (mod.sourcePath)
			this.sourceWatch = FileWatch.create(mod.sourcePath).subscribe(
				this.onWatch.bind(this)
			);

		if (mod.settings) this.registerSettings(mod.settings);
	}
	...
	unload() {
		if (this.mod.destroy) this.mod.destroy();
		// Invalidate require cache
		delete require.cache[this.resolvedPath];

		if (this.sourceWatch) this.sourceWatch.unsubscribe();
	}
	...
	onWatch() {
		...
		cxl.file.read(this.mod.sourcePath).then(
			d => {
				this.source = d;
...
class PluginManager extends ide.EventEmitter {
	 ...
	 */
	register(p) {
		if (p.id in this.plugins) throw `Plugin ${p.name} already registered.`;
	...
	getOnlinePackages() {
		return ide
			.exec('npm search --json -l =cxl "@cxl/workspace."')
			.then(
	...
	getPackages() {
		...
		return this.getOnlinePackages().then(all => {
			...
			cxl.each(plugins, function (a, k) {
				...
				} else plugin.error(`package.json not found for plugin "${k}"`);
			});
	...
	loadWorkspacePlugins() {
		var regex = /^workspace\./,
			dir =
				ide.configuration['plugins.path'] ||
		...
		if (fs.existsSync(dir)) {
			...
			data.forEach(d => {
				if (regex.test(d)) this.requireFile(path.join(dir, d));
			});
		}
	...
	getSources() {
		...
		for (i in this.plugins) {
			if (this.plugins[i].source) result += this.plugins[i].source + '\n';
		}

		if (this.scripts) result += this.scripts;

		if (!ide.configuration.debug)
			setImmediate(this.compileSources.bind(this));
		else this.sourcesUpdated();
...
plugin
	...
	.route('POST', '/plugins/uninstall', function (req, res) {
		ide.ServerResponse.respond(
			res,
			ide.plugins.uninstall(req.body.id),
			this
		);
	})

	.route('GET', '/plugins/tests', function (req, res, next) {
		if (!ide.configuration.debug) next();
	...
	.route('POST', '/plugins/enable', function (req, res) {
		var p = ide.projectManager.getProject(req.body.project);
		ide.ServerResponse.respond(
			res,
			ide.plugins.enable(p, req.body.name),
			this
		);
	})
	...
	.route('POST', '/plugins/disable', function (req, res) {
		...
		ide.ServerResponse.respond(
			res,
			ide.plugins.disable(p, req.body.name),
			this
		);
	})
