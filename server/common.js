...
class Configuration {
	...
	loadFile(fn, required) {
		...
		if (obj) ide.extend(this, obj);
		else if (required) throw `Could not read JSON file ${fn}`;
...
function respondStat(dir, file) {
	return cxl.file.stat(dir + '/' + file).then(
		function (stat) {
			return {
				filename: file,
				// TODO
...
class File {
	...
	static fromSocket(data) {
		...
		return file.read().then(function (file) {
			if (data.diff) File.patch(file.content, data.diff);
	...
	static list(dir, ignore) {
		return cxl.file.readDirectory(dir).then(function (list) {
			return Promise.all(
				list.reduce(function (result, file) {
	 ...
	 */
	read(encoding) {
		return File.stat(this.path)
			.then(stat => {
				this.stat = stat;
				this.encoding = encoding;

				if (stat.isNew) return '';
	...
	write(content) {
		...
		return Promise.all(resolve)
			.then(() =>
				cxl.file.write(this.path, this.content, { encoding: null })
			)
			.then(() => {
					...
					if (!(this.ignore && this.ignore(file))) {
						...
						fs.stat(this.root + '/' + relfile, (err, stat) => {
							if (!err) {
								...
								this.$results.push(
									this.serialize(relfile, stat)
								);
							} else console.log('Error ', relfile, err);

							if (--this.$pending === 0) this.$resolve();
						});
					}
				});
			else console.log('Error ', dir, err);
...
class FileMatcher {
	...
	constructor(files) {
		...
		if (files) this.push(files);
	}

	push(files) {
		if (Array.isArray(files)) files.forEach(this.push, this);
...
class FileManager {
	...
	watchFiles() {
		...
		this.log.dbg(
			`Watching ${
				files.length
			} directories. ${FileWatch.getCount()} Total`
		);
	}
}
...
class AssistServer {
	...
	$doInline(req) {
		...
		if (hints) req.respondInline(hints);
	}

	$doExtended(req) {
		var hints = this.extendedAssist(req, req.respondExtended.bind(req));

		if (hints) req.respondExtended(hints);
	}
...
class JSONRPC {
	...
	$onResponse(data) {
		...
		var match = data.match(/Content-Length:\s+(\d+)(?:.*)\r\n\r\n(.+)/),
			length,
			json,
			response,
			request;
		...
		if (json.id !== undefined) {
			...
			if (json.result) request.resolve(json.result);
			else if (json.error) request.reject(json.error);
		}
...
class Theme {
	constructor(p) {
		this.path = path.isAbsolute(p)
			? p
			: ide.basePath + '/public/theme/' + p + '.css';
		this.source = null;
		this.observer = new FileWatch(this.path).subscribe(
...
class LanguageServerStdIO extends AssistServer {
	...
	inlineAssist(request) {
		...
		else this.requestCompletion(request);
	}
...
class LanguageServer extends AssistServer {
	...
	$onInlineAssist(request) {
		if (this.canAssist(request)) this.onInlineAssist(request);
	}
}
...
class Process {
	constructor(command, parameters, options) {
		if (command) this.spawn(command, parameters, options);
	}
	...
	get stream() {
		return (
			this.$stream || (this.$stream = this.$createStream(this.$process))
		);
	}
...
class RPCServer {
	...
	$onError(client, data, err) {
		if (data.id)
			ide.socket.respond(client, this.pluginName, {
				id: data.id,
				error: {
					code: err.code,
					message: err.message,
	...
	$onSocket(client, data) {
		...
		if (id) this.$respond(client, id, result);
	}
}
...
class ResourceManager {
	...
	destroy() {
		this.resources.forEach(r =>
			r.destroy ? r.destroy() : r.unsubscribe()
		);
	}
}
...
module.exports = {
	...
	assist: {
		...
		findObject(obj, cursorValue, fn) {
			...
			for (i in obj)
				if ((match = this.match(i, cursorValue))) {
					if (fn) match = fn(match, obj[i], i, cursorValue);
		...
		findArray(array, cursorValue, fn) {
			var i = 0,
				...
				result = [],
				match,
				value = cursorValue.trimLeft(),
				prefix;

			if (value !== cursorValue) prefix = cursorValue;

			for (; i < l; i++)
				if ((match = this.match(array[i], value, prefix))) {
					if (fn) match = fn(match, array[i], i, value);
					if (match) result.push(match);
				}
	...
	extend(obj, p) {
		...
		for (var i in p) {
			...
			if (Array.isArray(c)) {
				if (Array.isArray(val)) cxl.pushUnique(c, val);
				else c.push(val);
			} else if (
				c &&
				typeof c === 'object' &&
